...
export const createCampaign = (title, category, excerpt, month, day, event_date, event_location, content) => async dispatch => {
    const config = {
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
    ...
    const body = JSON.stringify({ title, category, excerpt, month, day, event_date, event_location, content });

    try {
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/campaign/create`, body, config);
